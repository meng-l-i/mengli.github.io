$(function () {
    ...
    var jss = 0;
    $('#newinput').click(res => {
        if ($('#userinput').val() == null || $('#passwordinput').val() == null)//null
            alert("请输入账户和密码！");
        else if (localStorage.getItem("user-"+$('#userinput').val()) != null) {
            alert("该账户已注册");
    }
        else {
            if ($('#again-passwordinput').val() == null) {
                alert('请再一次输入密码！');
            }
            else if ($('#again-passwordinput').val() != $('#passwordinput').val()) {
                alert("两次密码不同！");
            }
            else {
                alert("成功新建用户。");
                localStorage.setItem('user-' + $('#userinput').val(), $('#passwordinput').val());
